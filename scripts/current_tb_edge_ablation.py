#!/usr/bin/env python3
"""Evaluate intra-TB pair weighting using the trained dE=1 edge checkpoint.

Modes:
  dynamic   - recompute the pair state from current node states every CGNN round,
              forcing previous-round edge input to zero each round.
  recurrent - reset edge at every TB boundary, but pass/refine it across CGNN rounds.

No edge state is ever carried across TBs in either mode.
"""
...
def parse_args():
    ...
    p.add_argument("--mode", required=True, choices=["dynamic", "recurrent"])
    ...
    p.add_argument("--seed", type=int, default=20261811)
    return p.parse_args()
...
class IntraTBEdgeCGNN(tf.keras.Model):
    """Exact dE=1 trained edge architecture with selectable intra-TB recurrence."""
    ...
    def call(self, inputs):
        ...
        edge = None  # always reset at TB boundary
        llrs = []
        h_hats = []
        for i in range(base._num_it):
            it = base._iterations[i]
            edge_in = None if self.mode == "dynamic" else edge
            a, edge_new = self._aggregate(s, active_tx, edge_in, it._state_aggreg)
            edge = edge_new
...
def forward(receiver, model, y, h_hat, active_tx):
    ...
    llrs, h_hats, edge = model([y2, pe, h_hat, active, mcs_mask])
...
def build_model():
    ...
    model = IntraTBEdgeCGNN(base, d_s=p.d_s, mode=ARGS.mode, name="temporal_edge_d1")
    return p, e2e, model
    ...
    for ebno in np.arange(1.0, 4.01, 0.5):
        ...
        for _ in range(ARGS.eval_max_batches):
            ...
            for t in range(8):
                ...
                if t > 0:  # match the previous edge-memory evaluation exactly
                    err = tf.reduce_any(
                        tf.not_equal(b, tf.cast(b_hat > 0.5, b.dtype)), axis=-1).numpy()
                    errors += int(err.sum())
                    blocks += err.size
        ...
        print(f"EVAL mode={ARGS.mode} ebno={ebno:.1f} tbler={tbler:.6f} ({errors}/{blocks})", flush=True)
        if tbler < 0.015:
            break
    return res
...
def latency(seq, p, model):
    ...
    def infer(y_, ls_, active_):
        llr_, _, edge_ = forward(seq._rx, model, y_, ls_, active_)
        return llr_, edge_

    for _ in range(20):
        z, e = infer(y, ls, active)
        _ = z.numpy(); _ = e.numpy()
...
def main():
    ...
    path = OUT / f"{ARGS.mode}_k{ARGS.num_it}.json"
    with open(path, "w") as f:
        json.dump(out, f, indent=2)
    print("RESULT", json.dumps({
        "mode": ARGS.mode,
        "crossing": out["snr_at_10pct_tbler_db"],
