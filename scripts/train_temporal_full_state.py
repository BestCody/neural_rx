...
def make_losses(receiver, model, manager, batch):
    ...
    for t in range(seq_len):
        state, prev, gap, valid = manager.gather(state, batch["ue_ids"][:, t], t)
        ...
        data_losses.append(tf.reduce_mean(
...
def temporal_gradient_check(receiver, model, manager, generator):
    ...
    with tf.GradientTape() as tape:
        ...
        _, _, u0 = temporal_forward(
            receiver, model,
            batch["y"][:, 0], batch["ls"][:, 0], batch["active"][:, 0],
            m0, g0, v0, training=True)
        state = manager.scatter(
            state, batch["ue_ids"][:, 0], u0, batch["active"][:, 0], 0)
        state, m1, g1, v1 = manager.gather(state, batch["ue_ids"][:, 1], 1)
        coded1 = receiver._tb_encoders[0](batch["bits"][:, 1])
        llr1, _, _ = temporal_forward(
            receiver, model,
            batch["y"][:, 1], batch["ls"][:, 1], batch["active"][:, 1],
            m1, g1, v1, training=True)
        loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
            labels=tf.cast(coded1, tf.float32), logits=llr1))
    grads = tape.gradient(loss, model.memory_variables)
        ...
        if step % ARGS.log_every == 0 or step == ARGS.train_steps - 1:
            row = {
                ...
                "memory_valid_fraction_per_tb": [
                    float(x.numpy()) for x in valid_fracs],
