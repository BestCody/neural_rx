...
class DynamicPairAggregation(Layer):
    ...
    def __init__(self, original_agg, d_s, **kwargs):
        ...
        # Zero logits => exact equal weighting at initialization.
        self.score_out = Dense(
            1, activation=None,
    ...
    def call(self, inputs):
        ...
        # Reuse the pretrained sender transform from the original NRX aggregator.
        sp = s
        for layer in self.original_agg._hidden_layers:
            sp = layer(sp)
        sp = self.original_agg._output_layer(sp)

        # One compact summary per UE, recomputed from the current CGNN state.
        ...
        logits = tf.squeeze(logits, axis=-1)  # [B,U,U]
        ...
        # Renormalize after masking; single-user case safely becomes all zeros.
...
def build_model(training, weighted):
    ...
    # Build all original NRX weights using four users, then load the shipped NRX Large checkpoint.
    ...
    if weighted:
        for i in range(ARGS.k):
            ...
            wrapped = DynamicPairAggregation(
                it._state_aggreg, p.d_s, name=f"dynamic_pair_agg_{i}")
            it._state_aggreg = wrapped
            pair_layers.append(wrapped)
        # Build the newly introduced score heads.
        _ = model(1, 4.0, num_tx=NUM_TX)
...
def train_variant(name, weighted):
    ...
    # Same stochastic training stream for cold and weighted models.
    ...
    for step in range(ARGS.train_steps):
        ...
        if weighted:
            ...
            base_pairs = [(g, v) for g, v in zip(grads, all_vars)
                          if g is not None and id(v) not in pids]
            pair_pairs = [(g, v) for g, v in zip(grads, all_vars)
                          if g is not None and id(v) in pids]
            if base_pairs:
    ...
    return {
        ...
        "pair_parameters": int(sum(np.prod(v.shape) for layer in pair_layers for v in layer.pair_variables)),
    }
...
def eval_variant(name, weighted, ckpt):
    ...
    # Broad sweep first; four-UE UMi operating point is not known a priori.
    ...
    # Pair-weight diagnostic at 4 dB: entropy and departure from equal 1/3 weighting.
    pair_diag = None
    if weighted:
        ...
        if vals:
            ...
            pair_diag = {
                "mean_offdiag_weight": float(np.mean(z)),
                "std_offdiag_weight": float(np.std(z)),
                "min_offdiag_weight": float(np.min(z)),
                "max_offdiag_weight": float(np.max(z)),
                "equal_weight_reference": 1.0 / 3.0,
            }

    return {
        "tbler_vs_ebno": res,
        "snr_at_10pct_tbler_db": crossing(res),
        "pair_weight_diagnostic": pair_diag,
    }
...
def main():
    ...
    out = {
        ...
        "channel": "UMi",
        "k": ARGS.k,
        "training_steps_each": ARGS.train_steps,
        "cold": {"train": cold_train, "eval": cold_eval},
        "weighted": {"train": weighted_train, "eval": weighted_eval},
    }
    ...
    print("RESULT", json.dumps({
        ...
        "cold_crossing": c,
        "weighted_crossing": w,
        "weighted_gain_db": out["weighted_gain_db"],
        "pair_parameters": weighted_train["pair_parameters"],
        "pair_diag": weighted_eval["pair_weight_diagnostic"],
    }), flush=True)
